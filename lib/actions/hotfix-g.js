...
const startQuestions = [{
        ...
        transformer: (value) => `hotfix-g/${value}`
    }];
...
async function start(hotfixName) {
    ...
    console.log('即将拉取灰度的 hotfix 分支 ->', chalk_1.default.yellowBright(hotfixName));
    try {
        // 1.更新 gray 分支
        git_1.Git.checkoutBranch('gray');
        await git_1.Git.pullWithRebase();
        // 2. 创建 hotfix-g 分支
        await git_1.Git.createBranchFrom(hotfixName, 'gray');
        await git_1.Git.checkoutBranch(hotfixName);
        console.log('hotfix-g 分支创建完成，当前所在分支 ->', chalk_1.default.yellowBright(hotfixName));
    }
    catch (e) {
        console.error(chalk_1.default.redBright('hotfix-g 分支创建失败'));
    }
}
exports.start = start;
async function finish(hotfixName) {
    ...
    try {
        git_1.Git.checkoutBranch('gray');
        await git_1.Git.pullWithRebase();
        await git_1.Git.mergeBranches('gray', hotfixName);
        console.info(chalk_1.default.green(`灰度的 hotfix 分支【${hotfixName}】合入 gray 完成`));
        git_1.Git.checkoutBranch('rc');
        await git_1.Git.pullWithRebase();
        await git_1.Git.mergeBranches('rc', 'gray');
        console.info(chalk_1.default.green(`灰度的 hotfix 分支【${hotfixName}】合入 rc 完成`));
        git_1.Git.checkoutBranch('develop');
        await git_1.Git.pullWithRebase();
        await git_1.Git.mergeBranches('develop', 'rc');
        console.info(chalk_1.default.green(`灰度的 hotfix 分支【${hotfixName}】合入 develop 完成`));
        console.info(chalk_1.default.green(`灰度环境的 hotfix 分支【${hotfixName}】合入 gray 完成，请手动 push [gray/rc/develop] 的代码`));
        ...
        if (confirm === 'y' || confirm === 'Y') {
            try {
                ...
                console.error(chalk_1.default.redBright(`hotfix-g【${hotfixName}】删除失败：${e}`));
            }
        }
    }
    catch (e) {
